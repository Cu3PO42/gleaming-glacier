{config, lib, inputs, flake-parts-lib, origin, ...}: with lib; flake-parts-lib.mkTransposedPerSystemModule {
  ...
  option = mkOption {
    type = (evalModules {
      modules = [
        origin.homeManagerModules.copper-chroma
        { _module.check = false; }
      ];
  ...
  imports = [{
    ...
    config = {
      copper.chroma.modules = lib.mkDefault (map ({value, ...}: value) (filter ({name, value}: origin.lib.startsWith "copper-chroma/" name) (attrsToList origin.homeManagerModules)));
    };
  }];
}